...
*   **Formula:** $\min_\phi \{E}_{x \sim p_{data}} [ D_{KL}( \ q_\phi(z|x) \ || \ p(z|x) \ ) ]$
...
*   **Role:** This is used to train the **Encoder**. It minimizes the **Inference Error**—the gap between the approximate posterior $q(z|x)$ and the true posterior $p(z|x)$.
